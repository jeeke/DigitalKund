package co.p3cs.digitalkund.data.repos
...
import co.p3cs.digitalkund.ui.auth.AuthActivity
    ...
    fun login(email: String, password: String, callback: RepositoryCallback<String>) {
        FirebaseAuth.getInstance().signInWithEmailAndPassword(email, password)
            .addOnCompleteListener { task ->
                ...
                when {
                    currentUser()?.isEmailVerified == true -> callback.onSuccess()
                    task.isSuccessful -> callback.onError("Please verify your mail !")
